...
// ═══════════════════════════════════════
// PromptPay QR Code Generator (BOT EMVCo spec)
// ═══════════════════════════════════════
function promptPayPayload(id, amount) {
  id = id.replace(/[^0-9]/g, '');
  const aidTag = id.length >= 13 ? '02' : '01';
  const aidVal = id.length >= 13 ? id : ('0066' + id);
  const merchant = tlv('29', tlv('00', 'A000000677010111') + tlv(aidTag, aidVal));
  let payload = tlv('00', '01') + tlv('01', amount ? '12' : '11') + merchant + tlv('53', '764') + tlv('58', 'TH');
...
function renderPromptPayQR(containerId, promptPayId, amount, size) {
  const el = document.getElementById(containerId);
  if (!el || !promptPayId) return;
  if (typeof QRCode === 'undefined') { el.innerHTML = '<p style="color:#94a3b8;font-size:.85rem">QR Code library ไม่พร้อมใช้งาน</p>'; return; }
  el.innerHTML = '';
  new QRCode(el, { text: promptPayPayload(promptPayId, amount), width: size || 200, height: size || 200, correctLevel: QRCode.CorrectLevel.M });
}
